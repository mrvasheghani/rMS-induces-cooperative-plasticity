...
simConfig.duration =    330*1e3        # Duration of the simulation, in ms  #72*1e3
...
def Simulation(amp_pre, amp_post):
    simConfig.filename='rMS-10Hz-Continuous-noisy-pre-'+str(amp_pre)+'post(0ms)-'+str(amp_post)
    # COUNT=0

    start_time=   60000
    for ttt in range (0,10000000,10000000):
        for tt in range(0,100000000,100000000):
            for t in range(0,90000,100):
...
for chunk in chunks:
    ...
    for i in range (chunk_size):
        ...
        filename='10Hz-pre-'+str(pre_a)+'post-'+str(post_a)
        ...
        plt.axvline(150, color='gray',linewidth=0.2)

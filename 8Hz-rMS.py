...
###############################################################################
# STDPparams = {'STDPon':1, 'hebbwt': 0.0001, 'antiwt':-0.0001, 'wmax': 50, 'RLon': 0 ,'tauhebb': 10, 'RLwindhebb': 0, 'useRLexp': 0, 'softthresh': 0, 'verbose':1}
...
simConfig.duration =    353*1e3        # Duration of the simulation, in ms  #72*1e3
...
def Simulation(amp_pre, amp_post):
    simConfig.filename='rMS-8Hz-Continuous-noisy-pre-'+str(amp_pre)+'post(0ms)-'+str(amp_post)
    # COUNT=0

    start_time=   60000
    for ttt in range (0,10000000,10000000):
        for tt in range(0,100000000,100000000):
            for t in range(0,112500,125):
...
for chunk in chunks:
    ...
    for i in range (chunk_size):
        ...
        filename='z-8Hz-pre-'+str(pre_a)+'post-'+str(post_a)
        ...
        plt.axvline(173, color='gray',linewidth=0.2)
